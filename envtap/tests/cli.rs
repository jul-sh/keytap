...
#[test]
fn ssh_keys_work_as_public_keys_and_identities() {
    let project = Project::new();
    let key_path = project.root.join("sam_ed25519");
    let generated = Command::new("ssh-keygen")
        .args(["-q", "-t", "ed25519", "-N", "", "-C", "sam@laptop", "-f"])
        .arg(&key_path)
        .output();
    let Ok(generated) = generated else {
        eprintln!("ssh-keygen is unavailable; skipping");
        return;
    };
    assert!(generated.status.success());
    let public = fs::read_to_string(project.root.join("sam_ed25519.pub")).unwrap();
    let mut tokens = public.split_whitespace();
    let (kind, key) = (tokens.next().unwrap(), tokens.next().unwrap());

    project.set(&project.owner, "SHARED", "for-sam");
    let mut command = project.owner();
    command.args(["grant", "sam", kind, key]);
    assert_success(&output(&mut command));
    assert!(project
        .text()
        .contains(&format!("#: grant sam {kind} {key} ")));

    let mut command = project.cli();
    command.arg("-i").arg(&key_path).args(["get", "SHARED"]);
    let result = output(&mut command);
    assert_success(&result);
    assert_eq!(stdout(&result), "for-sam\n");

    let mut command = project.cli();
    command.arg("-i").arg(&key_path).arg("public-key");
    let result = output(&mut command);
    assert_success(&result);
    assert_eq!(stdout(&result), format!("{kind} {key}\n"));

    let mut command = project.owner();
    command.args(["grant", "again", &format!("{kind} {key} other@comment")]);
    assert_failure(&output(&mut command), "already has access as `sam`");

    let rsa_path = project.root.join("rsa_key");
    let generated = Command::new("ssh-keygen")
        ...
        .unwrap();
    assert!(generated.status.success());
    let rsa_public = fs::read_to_string(project.root.join("rsa_key.pub")).unwrap();
    let mut command = project.owner();
    command.args(["grant", "rsa-user", rsa_public.trim()]);
    assert_failure(&output(&mut command), "ssh-rsa keys are not supported");
    let mut command = project.cli();
    command.arg("-i").arg(&rsa_path).args(["get", "SHARED"]);
    assert_failure(
        &output(&mut command),
        "ssh-rsa key, which Envtap does not support",
    );

    fs::remove_file(project.root.join("sam_ed25519.pub")).unwrap();
    let mut command = project.cli();
    command.arg("-i").arg(&key_path).arg("public-key");
    let result = output(&mut command);
    assert_success(&result);
    assert_eq!(
        stdout(&result),
        format!("{kind} {key}\n"),
        "the public key comes from the private key file"
    );
}
