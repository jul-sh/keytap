//! Human-verifiable binding for a nearby ceremony.
//!
...
//! controls two different encrypted sessions from grinding either transcript
...
impl Context {
    /// Freeze both the encrypted session and the exact request that the
    /// approver will execute after both people confirm the comparison words.
...
impl Comparison {
    ...
    fn write_prompt(&self, writer: &mut impl Write) -> Result<(), String> {
        write!(
            writer,
            "\nDoes your nearby device show exactly \"{}\"? [y/N, 2 minute timeout] ",
            self.phrase
        )
