...
//! through the decryptor) without needing a passkey. They guard the crypto core
//! of the v5 streaming rewrite: what encrypt writes, decrypt must recover.

use std::io::Read;
use std::str::FromStr;

fn identity_from_raw(raw: &[u8]) -> age::x25519::Identity {
    let sk = keytap_core::format_private_key(raw, keytap_core::PrivateKeyFormat::AgeSecretKey).unwrap();
...
fn stream_encrypt(recipient: &age::x25519::Recipient, plaintext: &[u8]) -> Vec<u8> {
    let enc = age::Encryptor::with_recipients(std::iter::once(recipient as &dyn age::Recipient)).unwrap();
    let mut ct = Vec::new();
...
fn stream_decrypt(identity: &age::x25519::Identity, ciphertext: &[u8]) -> Vec<u8> {
    ...
    let mut r = dec.decrypt(std::iter::once(identity as &dyn age::Identity)).unwrap();
    let mut out = Vec::new();
...
fn round_trips_small_payload() {
    ...
    let plaintext = b"hello keytap v5 streaming round trip";
...
fn round_trips_large_payload_streaming() {
    // 8 MiB — comfortably exercises chunked streaming without the old in-memory
    // ceiling being relevant. (The point is that no MAX_FILE_SIZE gate exists.)
    let raw = [42u8; 32];
    let id = identity_from_raw(&raw);
...
fn wrong_identity_fails_to_decrypt() {
    ...
    assert!(result.is_err(), "decrypting with the wrong identity must fail");
}
